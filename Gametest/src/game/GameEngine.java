...
public class GameEngine implements Runnable {
	...
	public void run() {
		...
		try {
			...
			guiObjects.add(brett1.getTargets().get(1));
			...
			for (int i = -900; i < 3000; i++) {
				...
				brett1.getTargets().get(1).setXLocation(brett1.getTargets().get(1).getXLocation()-(1*radioslol*((Math.sin((double)(i*((2*Math.PI)/300)))))));
				brett1.getTargets().get(1).setYLocation(brett1.getTargets().get(1).getYLocation()-(1*radioslol*((Math.cos((double)(i*((2*Math.PI)/300)))))));
